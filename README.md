...
| 31 | TS01 | Obtener lista de empresas | Como desarrollador, quiero obtener una lista de empresas a través del endpoint RESTful `/api/v1/companies` para poder mostrar la información de las empresas en el frontend. | Escenario 1: Solicitar todas las empresas. Dado que el endpoint `/api/v1/companies` está disponible, Cuando se realiza una solicitud GET, Entonces la respuesta debe tener estado 200 y el cuerpo debe incluir una lista de empresas. <br><br>Escenario 2: Respuesta vacía si no hay empresas. Dado que no existen empresas en la base de datos, Cuando se realiza una solicitud GET, Entonces la respuesta debe tener estado 200 y un cuerpo vacío. | EP06 |
| 32 | TS02 | Crear una nueva empresa | Como desarrollador, quiero crear una nueva empresa a través del endpoint RESTful `/api/v1/companies` para que las empresas puedan ser registradas en la base de datos. | Escenario 1: Crear empresa con datos válidos. Dado que el endpoint está disponible, Cuando se envía una solicitud POST con los datos válidos, Entonces la respuesta debe tener estado 201 y el cuerpo debe incluir la nueva empresa creada. <br><br>Escenario 2: Crear empresa con nombre vacío. Dado que se envía una solicitud POST con un campo name vacío, Entonces la respuesta debe tener estado 400 con un mensaje de error. | EP06 |
| 33 | TS03 | Obtener empleados de una empresa | Como desarrollador, quiero obtener la lista de empleados de una empresa a través del endpoint RESTful `/api/v1/companies/{companyId}/employees` para mostrar los empleados en el frontend. | Escenario 1: Obtener empleados de una empresa existente. Dado que el endpoint está disponible, Cuando se realiza una solicitud GET con un `companyId` válido, Entonces la respuesta debe tener estado 200 y una lista de empleados. <br><br>Escenario 2: Respuesta vacía si no hay empleados. Dado que no existen empleados para la empresa, Cuando se realiza una solicitud GET, Entonces la respuesta debe tener estado 200 y un cuerpo vacío. | EP06 |
| 34 | TS04 | Crear nuevo empleado | Como desarrollador, quiero crear un nuevo empleado a través del endpoint RESTful `/api/v1/employees` para registrar empleados en la base de datos. | Escenario 1: Crear empleado con datos válidos. Dado que el endpoint está disponible, Cuando se envía una solicitud POST con los datos válidos, Entonces la respuesta debe tener estado 201 y el cuerpo debe incluir el nuevo empleado creado. <br><br>Escenario 2: Crear empleado sin nombre. Dado que se envía una solicitud POST sin el campo name, Entonces la respuesta debe tener estado 400 con un mensaje de error. | EP06 |
| 35 | TS05 | Obtener resúmenes mensuales | Como desarrollador, quiero obtener un resumen mensual de horas trabajadas de los empleados a través del endpoint RESTful `/api/v1/monthly-summaries` para mostrar la información en el frontend. | Escenario 1: Obtener resumen mensual de empleados. Dado que el endpoint está disponible, Cuando se realiza una solicitud GET, Entonces la respuesta debe tener estado 200 y una lista de resúmenes mensuales. <br><br>Escenario 2: Respuesta vacía si no hay resúmenes mensuales. Dado que no existen resúmenes en la base de datos, Cuando se realiza una solicitud GET, Entonces la respuesta debe tener estado 200 y un cuerpo vacío. | EP06 |
| 36 | TS06 | Crear un nuevo resumen mensual | Como desarrollador, quiero crear un nuevo resumen mensual de horas trabajadas a través del endpoint RESTful `/api/v1/monthly-summaries` para registrar los resúmenes en la base de datos. | Escenario 1: Crear resumen mensual con datos válidos. Dado que el endpoint está disponible, Cuando se envía una solicitud POST con los datos válidos, Entonces la respuesta debe tener estado 201 y el cuerpo debe incluir el nuevo resumen creado. <br><br>Escenario 2: Crear resumen con horas negativas. Dado que se envía una solicitud POST con valores negativos, Entonces la respuesta debe tener estado 400 con un mensaje de error. | EP06 |
| 37 | TS07 | Implementar autenticación JWT | Como desarrollador, quiero implementar la autenticación JWT para asegurar el acceso a las APIs protegidas de la aplicación. | Escenario 1: Login exitoso con credenciales válidas. Dado que el endpoint `/api/v1/login` está disponible, Cuando se realiza una solicitud POST con email y password válidos, Entonces la respuesta debe tener estado 200 y el cuerpo debe incluir un JWT válido. <br><br>Escenario 2: Error de login con credenciales inválidas. Dado que el email o password son inválidos, Cuando se realiza una solicitud POST, Entonces la respuesta debe tener estado 401 con un mensaje de error. | EP06 |
| 38 | TS08 | Registrar un nuevo usuario | Como desarrollador, quiero permitir que nuevos usuarios se registren a través del endpoint RESTful `/api/v1/auth/register` para crear una cuenta en la aplicación. | Escenario 1: Registro exitoso con datos válidos. Dado que se proporciona un nombre, email y contraseña válidos, Cuando se realiza una solicitud POST, Entonces la respuesta debe tener estado 201 y el cuerpo debe incluir los datos del nuevo usuario. <br><br>Escenario 2: Error de registro con email duplicado. Dado que el email ya está registrado, Cuando se realiza una solicitud POST, Entonces la respuesta debe tener estado 409 con un mensaje de error. | EP06 |
| 39 | TS09 | Obtener detalles de reportes de desempeño | Como desarrollador, quiero obtener los reportes de desempeño de los empleados a través del endpoint RESTful `/api/v1/reports` para mostrarlos en el frontend. | Escenario 1: Obtener lista de reportes. Dado que el endpoint está disponible, Cuando se realiza una solicitud GET, Entonces la respuesta debe tener estado 200 y una lista de reportes. <br><br>Escenario 2: Respuesta vacía si no hay reportes. Dado que no existen reportes en la base de datos, Cuando se realiza una solicitud GET, Entonces la respuesta debe tener estado 200 y un cuerpo vacío. | EP06 |
